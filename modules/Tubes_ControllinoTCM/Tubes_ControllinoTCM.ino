 ...
 ******************In this version DCU stands for TCM 1601 controller***********************
 */
#include <SPI.h>
...
#define DCU_STARTSTOP_ADDR 49 //(DCU ON/OFF Command)
#define DCU_STANDBY_ADDR 50  //(DCU STANDBY Command)
#define DCU_END_ADDR 51
...
#define XGS_EMULT1_ADDR 56   //(XGS EMULT 1 ON/OFF Command)
#define XGS_EMULT2_ADDR 57   //(XGS EMULT 2 ON/OFF Command)
#define XGS_DEGAS_ADDR 58   //(XGS DEGAS ON/OFF Command)
#define XGS_END_ADDR 59 
      ...
      if (serData6.length() == 20) { // Data OK
       ...
       if ( (csum%256) == csumd) { // Data OK
        ...
        if ( serData6.substring(5, 8) == serData5.substring(5, 8)) { // Channel OK
          ...
          else if (i == 3 || i == 4 || i == 5 || i == 6) { // Boolean value
  ...
  for (int i = 0 ; i < 4 ; i++) {
    ...
    if (serData8 == "") {// Com Status Error
      ...
      if ( i == 0 ) { // Pressure float
        ...
        String prval = serData8;
        uint16_t result=0;
        sscanf(serData8.c_str(),"%d",&result); // 00 -> OFF; 01 -> ON // 01 -> Fil1; 02 -> Fil2
        mb_mapping.tab_holding_registers[curaddr++] = result;
      }
    }
 }
...
{    
 ...
 /* Serial.print("PIN4_VALVE=");Serial.print(digitalRead(PIN4_VALVE));Serial.print(" HIGH=");
  ...
  if (digitalRead(COMPRESS_AIR_STATUS) == HIGH)
     mb_mapping.tab_holding_registers[COMPRESSAIR_STATUS_ADDR] = 0x01; // COMPRESS AIR ON
  else if (digitalRead(COMPRESS_AIR_STATUS) == LOW)
     mb_mapping.tab_holding_registers[COMPRESSAIR_STATUS_ADDR] = 0x00; // COMPRESS AIR OFF 
  else
