...
const byte ip[] = { 192, 168, 224, 172 }; // Controllino-20 has this IP
...
SoftwareSerial RS485Serial(CONTROLLINO_D9, CONTROLLINO_PIN_HEADER_DIGITAL_OUT_13); // RX, TX for Controllino (TCM Pfeiffer Turbo)
SoftwareSerial Spare1Serial(CONTROLLINO_D10, CONTROLLINO_PIN_HEADER_DIGITAL_OUT_14); // RX, TX for Controllino (V81-AGVarian Turbo)
SoftwareSerial Spare2Serial(CONTROLLINO_D8, CONTROLLINO_PIN_HEADER_DIGITAL_OUT_12); // RX, TX for Controllino (MaxiGauge 2)
...
#define MG1_START_ADDR 0
#define MG1_START_CMD_ADDR 24 //(MAXIGAUGE1 ON/OFF Command starting point)
#define MG1_END_ADDR 30
#define MG2_START_ADDR MG1_END_ADDR+1
#define MG2_START_CMD_ADDR 55 //(MAXIGAUGE2 ON/OFF Command starting point)
...
#define DCU_START_ADDR MG2_END_ADDR+8
#define DCU_STARTSTOP_ADDR 78 //(DCU ON/OFF Command)
#define DCU_STANDBY_ADDR 79  //(DCU STANDBY Command)
#define DCU_END_ADDR 80
#define VARIAN_START_ADDR DCU_END_ADDR+1
#define VARIAN_STARTSTOP_ADDR 89 //(VARIAN ON/OFF Command)
#define VARIAN_LOWSPEED_ADDR 90  //(VARIAN LOWSPEED Command)
#define VARIAN_END_ADDR 91
 ...
 */
  ...
  if (digitalRead(V21_CLOSE_VALVE) == LOW && V21_RESET == true) {
    if ( millis() - V21time > reset_wait) {
       ...
       mb_mapping.tab_holding_registers[V21_VALVE_ADDR] = 0x00;
       V21_RESET = false;  
    }
  }
 ...
 */
  ...
  if (digitalRead(V22_CLOSE_VALVE) == LOW && V22_RESET == true) {
    if ( millis() - V22time > reset_wait) {
       ...
       mb_mapping.tab_holding_registers[V22_VALVE_ADDR] == 0x00;
       V22_RESET = false;  
    }
  }
 ...
 */
  ...
  if (digitalRead(P22_OFF_SCROLL) == LOW && P22_RESET == true) {
    if ( millis() - P22time > reset_wait) {
      ...
      mb_mapping.tab_holding_registers[P22_SCROLL_ADDR] == 0x00;
      P22_RESET = false;
    }
  }
  // Reset P22_ON_SCROLL
...
void setup() {
  ...
  // Open serial communication for XGS-600 XGS Gauge
...
void loop() {
  ...
  for (int i = 0; i < 7; i++) {
     ...
     if (serData2 == "") {// Com Status Error - reset all values -
      for (int j = MG1_START_ADDR; j < MG1_END_ADDR; j++)
        mb_mapping.tab_holding_registers[j] = 0x00;
      mb_mapping.tab_holding_registers[MG1_END_ADDR] = 0x01;
      break;
     }
     else {
      mb_mapping.tab_holding_registers[MG1_END_ADDR] = 0x00;
      if (i < 6) { // Sensor pressures & status
        ...
        curaddr=MG1_START_ADDR - 1; // go back to initial position (- 1)
        for (int j = 0; j < 6 ; j++) {
          curaddr+=4;
  ...
  for (int i = 0; i < 7; i++) {
     ...
     if (i==6)
      serData1 = "SEN,0,0,0,0,0,0\r";
     Spare2Serial.listen();
     Spare2Serial.print(serData1);
     Serial.println(serData1);
     memset(buffer,0,length);
     Spare2Serial.readBytes(buffer,length);
     //Serial.println(buffer);
     serData2 = buffer;
     Spare2Serial.flush(); 
     // Communicate with Master (send/receive data)
     SendReceiveMaster();
     Spare2Serial.print("\x05");
     memset(buffer,0,length);
     Spare2Serial.readBytes(buffer,length);
     //Serial.println(buffer);
     serData2 = buffer;
     ...
     if (serData2 == "") {// Com Status Error - reset all values -
      for (int j = MG2_START_ADDR; j < MG2_END_ADDR; j++)
        mb_mapping.tab_holding_registers[j] = 0x00;
      mb_mapping.tab_holding_registers[MG2_END_ADDR] = 0x01;
      break;
     }
     else {
      mb_mapping.tab_holding_registers[MG2_END_ADDR] = 0x00;
      if (i < 6) { // Sensor pressures & status
        String prval = serData2.substring(serData2.indexOf(",")+1);
        mbfValue.fvalue = prval.toFloat();
        mb_mapping.tab_holding_registers[curaddr++] = mbfValue.value[0]; 
...
{    
  ...
  // LSB & MSB registers for float pressure values from MaxiGauge
   for (int i = MG1_START_ADDR; i < MG1_END_ADDR; i++)
...
{ 
  /***********************************************************************************************************/
  /* Update data from Modbus Master */
  /***********************************************************************************************************/
  ...
  /***********************************************************************************************************/
  if (mb_mapping.tab_holding_registers[V21_VALVE_ADDR] == 0x02 && V21_RESET == false) {
     digitalWrite(V21_CLOSE_VALVE,LOW); // CLOSE VALVE
     V21time = millis();
     V21_RESET = true;
  }
  else if (mb_mapping.tab_holding_registers[V21_VALVE_ADDR] == 0x01 && V21_RESET == false) {
  ...
  /***********************************************************************************************************/
  if ( mb_mapping.tab_holding_registers[V22_VALVE_ADDR] == 0x02 && V22_RESET == false) {
     digitalWrite(V22_CLOSE_VALVE,LOW);   // CLOSE VALVE
     V22time = millis();
     V22_RESET = true;
  }
  else if ( mb_mapping.tab_holding_registers[V22_VALVE_ADDR] == 0x01 && V22_RESET == false) {
  ...
  /***********************************************************************************************************/
  if ( mb_mapping.tab_holding_registers[P22_SCROLL_ADDR] == 0x02 && P22_RESET == false) {
     digitalWrite(P22_OFF_SCROLL,LOW);     // SWITCH OFF SCROLL
     P22time = millis();
     P22_RESET = true; 
  }
  else if ( mb_mapping.tab_holding_registers[P22_SCROLL_ADDR] == 0x01 && P22_RESET == false) { 
 ...
 /* Serial.print("PIN4_VALVE=");Serial.print(digitalRead(PIN4_VALVE));Serial.print(" HIGH=");
  ...
  /***********************************************************************************************************/
  for (int j = MG1_START_CMD_ADDR; j < MG1_END_ADDR; j++) {
    if ( mb_mapping.tab_holding_registers[MG1_END_ADDR] == 0x00 && mb_mapping.tab_holding_registers[j] != 0x00 ) {
  ...
  for (int j = MG2_START_CMD_ADDR; j < MG2_END_ADDR; j++) {
    if ( mb_mapping.tab_holding_registers[MG2_END_ADDR] == 0x00 && mb_mapping.tab_holding_registers[j] != 0x00 ) {
      /*Serial.print(" channel:");
      Serial.print(j);
      Serial.print(" set new register:");
      Serial.println(mb_mapping.tab_holding_registers[j]);
      */
      ...
      Spare2Serial.print(cmd);
      Serial.println(cmd);
      Spare2Serial.readString();
      Spare2Serial.print("\x05");
      Spare2Serial.readString();
      mb_mapping.tab_holding_registers[j] = 0x00;  // RESET register
    }
