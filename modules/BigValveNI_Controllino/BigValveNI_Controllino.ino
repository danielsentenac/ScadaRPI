...
#define COMPRESSAIR CONTROLLINO_A6 // COMPRESS AIR MEASURE
#define LN2TANKVALVE_NI_RELAY CONTROLLINO_R4  // NI LN2 TANK VALVE RELAY
#define LN2TANKVALVE_WI_RELAY CONTROLLINO_R5  // WI LN2 TANK VALVE RELAY
...
#define LN2TANKVALVE_NI_ADDR 6
#define LN2TANKVALVE_WI_ADDR 7
#define CRYO_TEMP_ADDR 8
#define COMPRESSAIR_ADDR 10
#define ARD_RESET_ADDR 12
...
void loop() {
  ...
  if ( millis() - temptime > temp_wait ) {
     ...
     Serial.print("Temperature=");Serial.print(tempbuf);Serial.print(" error=");Serial.println(error);
     if ( (error == 0 && (abs(tempbuf - conversion.fvalue) < tempDelta) || (error == 0 && startTempRead == true))) {
        conversion.fvalue = tempbuf; // Assert temp.fvalue is not erroneous
        startTempRead = false;
     }
     temptime = millis();
  }
...
{    
  ...
  digitalWrite(LN2TANKVALVE_NI_RELAY,LOW);                          // Set LN2TANKVALVE_NI_RELAY to LOW
  pinMode(LN2TANKVALVE_NI_RELAY,OUTPUT) ;                           // sets the digital relay pin as output for LN2 Tank Valve
  holdingRegisters[LN2TANKVALVE_NI_ADDR] = 0x01;    // Initialize RELAY ADDRESS to OPEN
  digitalWrite(LN2TANKVALVE_WI_RELAY,LOW);                          // Set LN2TANKVALVE_WI_RELAY to LOW
  pinMode(LN2TANKVALVE_WI_RELAY,OUTPUT) ;                           // sets the digital relay pin as output for LN2 Tank Valve
  holdingRegisters[LN2TANKVALVE_WI_ADDR] = 0x01;    // Initialize RELAY ADDRESS to OPEN

  holdingRegisters[CRYO_TEMP_ADDR] = 0x00;
  holdingRegisters[CRYO_TEMP_ADDR+1] = 0x00;

  holdingRegisters[COMPRESSAIR_ADDR] = 0x00;
  holdingRegisters[COMPRESSAIR_ADDR+1] = 0x00;
...
{    
  ...
  /***********************************************************************************************************/
  /* Update LN2 Tank Valve Relay NI (CLOSE/OPEN) */
  /***********************************************************************************************************/
  if (holdingRegisters[LN2TANKVALVE_NI_ADDR] == 0x02) {
     digitalWrite(LN2TANKVALVE_NI_RELAY,HIGH); // OPEN RELAY (CLOSE VALVE)
  }
  else if (holdingRegisters[LN2TANKVALVE_NI_ADDR] == 0x01) {
     digitalWrite(LN2TANKVALVE_NI_RELAY,LOW); // CLOSE RELAY (OPEN VALVE)
  } 
  /***********************************************************************************************************/
  /* Update LN2 Tank Valve Relay WI (CLOSE/OPEN) */
  /***********************************************************************************************************/
  if (holdingRegisters[LN2TANKVALVE_WI_ADDR] == 0x02) {
     digitalWrite(LN2TANKVALVE_WI_RELAY,HIGH); // OPEN RELAY (CLOSE VALVE)
  }
  else if (holdingRegisters[LN2TANKVALVE_WI_ADDR] == 0x01) {
     digitalWrite(LN2TANKVALVE_WI_RELAY,LOW); // CLOSE RELAY (OPEN VALVE)
  } 
  /***********************************************************************************************************/
  ...
  /***********************************************************************************************************/
  /* Update Compressair register */
  /***********************************************************************************************************/
  int compressair = analogRead(COMPRESSAIR);
  Serial.print("Compressair Level=");Serial.println(compressair);
  conversion.fvalue = (float) (compressair) * 16 / 426.; // (0 --> 16 bar scale + 10 bit analog pin + 0-24V --> 0-10V conversion)
  Serial.println(conversion.fvalue);
  holdingRegisters[COMPRESSAIR_ADDR] = conversion.value[0];
  holdingRegisters[COMPRESSAIR_ADDR+1] = conversion.value[1];
}
