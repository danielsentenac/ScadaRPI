...
long resetinstrument_wait = 4000;
...
#define MASTER_IN_STATUS CONTROLLINO_PIN_HEADER_DIGITAL_OUT_02 // MASTER IN STATUS
#define MASTER_OUT_STATUS CONTROLLINO_PIN_HEADER_DIGITAL_OUT_01 // MASTER OUT STATUS
...
void I2CupdateData() 
{
  ...
  FloatUint32 temp;
  int pos = 0;
  if (digitalRead(MASTER_IN_STATUS) == HIGH) {
    return; // Wait for next cycle
  }
  // MASTER available, lock it
  digitalWrite(MASTER_OUT_STATUS, HIGH);
  delay(100); // Wait double more than other MASTER to keep priority in case of arbitration
  if (digitalRead(MASTER_IN_STATUS) == LOW) { // Check if MASTER has priority
     ...
     if (numbyte == 12) { // Expect 8 bytes of data
        ...
        */
        ...
        if ( crc == crcdata ) {
           ...
           Serial.print("TEMPERATRURE 16 bit = ");
