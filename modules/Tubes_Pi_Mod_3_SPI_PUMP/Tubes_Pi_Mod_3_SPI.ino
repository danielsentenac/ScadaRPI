...
#define PUMP_START_CMD_BIT          8   // PUMP_START Open bit
#define PUMP_STOP_CMD_BIT           9   // PUMP_START Close bit
#define PUMP_STATUS_BIT             10  // PUMP Status bit
 ...
 // Reset PUMP_STOP_CMD
  if (digitalRead(PUMP_STOP_CMD) == LOW && PUMP_RESET == true) {
    if ( millis() - PUMPtime > reset_wait) {
       digitalWrite(PUMP_STOP_CMD,HIGH);  // RESET PUMP_STOP FAN
  ...
  if (digitalRead(PUMP_START_CMD) == HIGH && PUMP_RESET == true) {
    if ( millis() - PUMPtime > reset_wait) {
       digitalWrite(PUMP_START_CMD,LOW);   // RESET PUMP_START FAN
