...
IPAddress ip( 192, 168, 224, 161 ); // Controllino-9 has this IP
...
#define VARIAN_STARTSTOP_ADDR 17 //(VARIAN ON/OFF Command)
#define VARIAN_LOWSPEED_ADDR 18  //(VARIAN LOWSPEED Command)
#define VARIAN_END_ADDR 19
...
void loop() {
 ...
 /***********************************************************************************************************/
  ...
  /***********************************************************************************************************/
  Serial.println("Start reading Varian turbo-v 250");
  ...
  // Communicate with Master (send/receive data)
    SendReceiveMaster();
     ...
     switch (i) {
       case 0:
        serData5="\x49\xB7";  // Status
         break;
       case 1:
        serData5="\x4A\xB6";  // Speed, Temp, Voltage, Current
         break;
         case 2:
        serData5="\x4B\xB5";  // Pump life
         break;
     }
     delay(500);
     Spare1Serial.listen();
     ...
     for (int j = VARIAN_START_ADDR; j < VARIAN_END_ADDR; j++)
      ...
      uint16_t result=0;
      if ( i == 0 ) {
         serData6 = serData6.substring(0,2);
         unsigned char rep;
         rep = serData6[0];
         unsigned char lsb = rep & 0x0F;
         Serial.print("pump status=");Serial.println(lsb);
         holdingRegisters[curaddr++] = lsb;  // PUMP ON/OFF STATUS
      }
      if ( i == 1 ) {
         serData6 = serData6.substring(0,5);
         uint8_t rep[4];
         rep[0] = serData6[0]; // VOLTAGE
         rep[1] = serData6[1]; // CURRENT
         rep[2] = serData6[2]; // SPEED
         rep[3] = serData6[3]; // TEMP
         Serial.print("SPEED ");Serial.println((int)rep[2]); // SPEED
         Serial.print("TEMP ");Serial.println((int)rep[3]); // TEMP
         holdingRegisters[curaddr++] = rep[2];
         holdingRegisters[curaddr++] = rep[3];
         mbfvalue.fvalue = (float)(rep[0])*(float)(rep[1])*0.003844675; // POWER
         Serial.print("Power ");Serial.println(mbfvalue.fvalue);  
         holdingRegisters[curaddr++] = mbfvalue.value[0];
         holdingRegisters[curaddr++] = mbfvalue.value[1];
      }
      if ( i == 2 ) {
         serData6 = serData6.substring(4,8);
         
         mbuvalue.value[0] = serData6[0];
         mbuvalue.value[1] = serData6[1];
         mbuvalue.value[2] = serData6[2];
         mbuvalue.value[3] = serData6[3];
         Serial.print("PUMP HRS ");Serial.println(mbuvalue.uvalue);
         mbivalue.ivalue = mbuvalue.uvalue;
         holdingRegisters[curaddr++] = mbivalue.value[0];
         holdingRegisters[curaddr++] = mbivalue.value[1];
      }
     }
  }
  Serial.println("End reading Varian turbo-v 250");
...
{    
 ...
 /* if ( holdingRegisters[DCU_END_ADDR] == 0x00 && holdingRegisters[DCU_STARTSTOP_ADDR] != 0x00 ) {
  ...
  if ( holdingRegisters[VARIAN_STARTSTOP_ADDR] != 0x00 ) {
      String cmd="";;
      if ( holdingRegisters[VARIAN_STARTSTOP_ADDR] == 0x02 )
        cmd = "\x42\xBE";   // STOP COMMAND
      else if ( holdingRegisters[VARIAN_STARTSTOP_ADDR] == 0x01 )
        cmd = "\x41\xBF";  //START COMMAND
      Spare1Serial.listen();
      Spare1Serial.print(cmd);
      Serial.println(cmd);
      holdingRegisters[VARIAN_STARTSTOP_ADDR] = 0x00;  // RESET register
  ...
  if ( holdingRegisters[VARIAN_LOWSPEED_ADDR] != 0x00 ) {
      String cmd="";;
      if ( holdingRegisters[VARIAN_LOWSPEED_ADDR] == 0x02 )
        cmd =  "\x43\xBD";   // LOWSPEED OFF COMMAND
      else if ( holdingRegisters[VARIAN_LOWSPEED_ADDR] == 0x01 )
        cmd =  "\x44\xBC";  // LOWSPEED ON COMMAND
      Spare1Serial.listen();
      Spare1Serial.print(cmd);
      Serial.println(cmd);
      holdingRegisters[VARIAN_LOWSPEED_ADDR] = 0x00;  // RESET register
