/*
 * This Class is used for Cryo1500wStatusDetails GUI using Glg Toolkit
 */
...
public class GlgCryo1500wIntGui extends GlgChildGui {

    private static final long serialVersionUID = 354054054072L;
    private static final Logger logger = Logger.getLogger("Main");

    public GlgCryo1500wIntGui (GlgGui _parent, DeviceManager _deviceManager, String _title, String _origin) {
    ...
    protected void createAndShowGui () {
       ...
       try {
          ...
          // Cryo1500wStatusDetails display
          glg_bean.SetDrawingFile("Cryo1500wInt.g");
          // Add glg_bean component to a frame
	  content.add( glg_bean ); 
          glg_bean.AddListener( GlgObject.INPUT_CB, new InputListener() );
          glg_bean.SetSTag("title", title + " Internal", true);
    ...
    public void createCommandMap(Hashtable < String, String > CMD) {
       ...
       CMD.clear();
       CMD.putAll(Cryo1500wIntCMD);
    }

    public void createStatusMap(Hashtable < String, String > STATUS) {

       // Cryo1500wStatusDetails Gui case
       STATUS.clear();
       STATUS.putAll(Cryo1500wIntStatus);
    }
    ...
    protected void updateGuiFeatures(String glgName, DataElement dataElement) {
       ...
       if (glgName.contains("Bypass")) {
          ...
          if ( glgName.contains("Col") )// Col is a tag for object status (short type) color property
             glg_bean.SetGTag(glgName, FailOkColorSTATUS.get((int)dataElement.value), true);
             //logger.finer("GlgChildGui:updateGui> OkFailColorSTATUS=" + dataElement.value);
          if ( glgName.contains("Str") )  // Col is a tag for object status (short type) string property
             glg_bean.SetSTag(glgName, glgName.replace("Str", "") +" "+ FailOkSTATUS.get((int)dataElement.value), true);
             //logger.finer("GlgChildGui:updateGui> OkFailSTATUS=" + dataElement.value);       
       }
    }
	...
	public void InputCallback( GlgObject viewport, GlgObject message_obj )  {
	   ...
	   subaction = message_obj.GetSResource( "SubAction" );
           ...
           // Cryo800 ON command
           else if ((action.equals("MouseClick") ) && origintag.contains("Cryo800ON")) {
              String cmdName =  CMD.get("Cryo800ON");
              logger.finer("Origin=" + origintag + " Format=" + format + " Action=" + action + " subAction=" + subaction);
           ...
           // Cryo800 OFF command
           else if ((action.equals("MouseClick") ) && origintag.contains("Cryo800OFF")) {
              String cmdName =  CMD.get("Cryo800OFF");
              if ( cmdName == null ) return; 
              // Get corresponding device from deviceManager
              String deviceName = cmdName.split("_",2)[0];
              Device device = deviceManager.getDevice(deviceName);
           ...
           // Cryo900 ON command
           else if ((action.equals("MouseClick") ) && origintag.contains("Cryo900ON")) {
              String cmdName =  CMD.get("Cryo900ON");
              logger.finer("Origin=" + origintag + " Format=" + format + " Action=" + action + " subAction=" + subaction);
              logger.finer("cmdName=" + cmdName);
              if ( cmdName == null ) return; 
              // Get corresponding device from deviceManager
           ...
           // Cryo900 OFF command
           else if ((action.equals("MouseClick") ) && origintag.contains("Cryo900OFF")) {
              String cmdName =  CMD.get("Cryo900OFF");
              if ( cmdName == null ) return; 
              // Get corresponding device from deviceManager
              String deviceName = cmdName.split("_",2)[0];
              Device device = deviceManager.getDevice(deviceName);
           ...
           // Chiller ON command
           else if ((action.equals("MouseClick") ) && origintag.contains("ChillerON")) {
              String cmdName =  CMD.get("ChillerON");
              logger.finer("Origin=" + origintag + " Format=" + format + " Action=" + action + " subAction=" + subaction);
              logger.finer("cmdName=" + cmdName);
              if ( cmdName == null ) return; 
              // Get corresponding device from deviceManager
           ...
           // Chiller OFF command
           else if ((action.equals("MouseClick") ) && origintag.contains("ChillerOFF")) {
              String cmdName =  CMD.get("ChillerOFF");
              if ( cmdName == null ) return; 
              // Get corresponding device from deviceManager
              String deviceName = cmdName.split("_",2)[0];
              Device device = deviceManager.getDevice(deviceName);
           ...
           // Bypass Waterloop ON command
           else if ((action.equals("MouseClick") ) && origintag.contains("WaterloopON")) {
              String cmdName =  CMD.get("WaterloopON");
              logger.finer("Origin=" + origintag + " Format=" + format + " Action=" + action + " subAction=" + subaction);
              logger.finer("cmdName=" + cmdName);
              if ( cmdName == null ) return; 
              // Get corresponding device from deviceManager
           ...
           // Bypass Waterloop OFF command
           else if ((action.equals("MouseClick") ) && origintag.contains("WaterloopOFF")) {
              String cmdName =  CMD.get("WaterloopOFF");
              if ( cmdName == null ) return; 
              // Get corresponding device from deviceManager
              String deviceName = cmdName.split("_",2)[0];
              Device device = deviceManager.getDevice(deviceName);
