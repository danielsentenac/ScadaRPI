...
public class GlgTitane_32Gui extends GlgChildGui {

    private static final long serialVersionUID = 354054054075L;
    private static final Logger logger = Logger.getLogger("Main");

    public GlgTitane_32Gui (GlgGui _parent, DeviceManager _deviceManager, String _title, String _origin) {
    ...
    protected void createAndShowGui () {
       ...
       try {
          ...
          glg_bean.SetDrawingFile("Titane_32.g");
    ...
    protected void createStatusMap(Hashtable < String, String > STATUS) {
       ...
       // Titane Gui case
       STATUS.clear();
       STATUS.put("Pname", LINKSTATUS.get(origin));
       STATUS.put("P", LINKSTATUS.get(origin));
       STATUS.put("PCol", LINKSTATUS.get(origin));
       STATUS.put("PStr", LINKSTATUS.get(origin));
       STATUS.put("PStart", LINKSTATUS.get(origin + "Startsub"));
       STATUS.put("PStartCol", LINKSTATUS.get(origin + "Startsub"));
       STATUS.put("PStartStr", LINKSTATUS.get(origin + "Startsub"));
       STATUS.put("PCtrl", LINKSTATUS.get(origin + "Ctrlsub"));
       STATUS.put("PCtrlCol", LINKSTATUS.get(origin + "Ctrlsub"));
       STATUS.put("PCtrlStr", LINKSTATUS.get(origin + "Ctrlsub"));
       STATUS.put("PErr", LINKSTATUS.get(origin + "Errsub"));
       STATUS.put("PErrCol", LINKSTATUS.get(origin + "Errsub"));
       STATUS.put("PErrStr", LINKSTATUS.get(origin + "Errsub"));
       STATUS.put("PREmode", LINKSTATUS.get(origin + "REmodesub"));
       STATUS.put("PREmodeCol", LINKSTATUS.get(origin + "REmodesub"));
       STATUS.put("PREmodeStr", LINKSTATUS.get(origin + "REmodesub"));
       STATUS.put("PFILmode", LINKSTATUS.get(origin + "FILmodesub"));
       STATUS.put("PFILmodeCol", LINKSTATUS.get(origin + "FILmodesub"));
       STATUS.put("PFILmodeStr", LINKSTATUS.get(origin + "FILmodesub"));
       STATUS.put("POPmode", LINKSTATUS.get(origin + "OPmodesub"));
       STATUS.put("POPmodeCol", LINKSTATUS.get(origin + "OPmodesub"));
       STATUS.put("POPmodeStr", LINKSTATUS.get(origin + "OPmodesub"));
       STATUS.put("PCTRLOPmode", LINKSTATUS.get(origin + "CTRLOPmodesub"));
       STATUS.put("PCTRLOPmodeCol", LINKSTATUS.get(origin + "CTRLOPmodesub"));
       STATUS.put("PCTRLOPmodeStr", LINKSTATUS.get(origin + "CTRLOPmodesub"));
       STATUS.put("PAcurrentVal", LINKSTATUS.get(origin + "AcurrentValsub"));
       STATUS.put("PAvoltageVal", LINKSTATUS.get(origin + "AvoltageValsub"));
       STATUS.put("PSublcurrentVal", LINKSTATUS.get(origin + "SublcurrentValsub"));
       STATUS.put("PSubltimeVal", LINKSTATUS.get(origin + "SubltimeValsub"));
       STATUS.put("PSublperiodVal", LINKSTATUS.get(origin + "SublperiodValsub"));
       STATUS.put("PSublwaitVal", LINKSTATUS.get(origin + "SublwaitValsub"));
       STATUS.put("PCycleVal", LINKSTATUS.get(origin + "CycleValsub"));
       STATUS.put("PHrVal", LINKSTATUS.get(origin + "HrValsub"));
       STATUS.put("PBTempVal", LINKSTATUS.get(origin + "BTempValsub"));
    }

    public void updateGuiFeatures(String origintag, DataElement dataElement) {

        // Titane Pump Gui case
        //logger.finer("GlgTitane_32Gui:updateGuiFeatures> UPDATE " + dataElement.name + " (" + origintag + ") with value=" 
        //              + dataElement.value);
        ...
        if ( origintag.equals("PREmodeCol") ) {  // Col is a tag for object status (short type) color property
            glg_bean.SetGTag(origintag, TitaneRemoteColorSTATUS.get((int)dataElement.value), true);
        }
        if ( origintag.equals("PREmodeStr") ) { // Str is a tag for object status (short type) string property
            glg_bean.SetSTag(origintag,  TitaneRemoteSTATUS.get((int)dataElement.value), true);
        }
        ...
        if ( origintag.equals("POPmodeCol") ) {  // Col is a tag for object status (short type) color property
            glg_bean.SetGTag(origintag, TitaneOperatingModeColorSTATUS.get((int)dataElement.value), true);
        }
        if ( origintag.equals("POPmodeStr") ) { // Str is a tag for object status (short type) string property
            glg_bean.SetSTag(origintag,  TitaneOperatingModeSTATUS.get((int)dataElement.value), true);
        }
	...
	public void InputCallback( GlgObject viewport, GlgObject message_obj )  {
	   ...
	   subaction = message_obj.GetSResource( "SubAction" );
           logger.finer("GlgTitane_32Gui::InputListener> Origin=" + origintag + " Format=" + format + 
           ...
           // Titane ON/OFF command
              ...
              System.out.println("GlgTitane_32Gui::InputListener> Open Dialog command for " + origin + "," + origintag );
              String cmdName =  STATUS.get(origintag);
              System.out.println("GlgTitane_32Gui::InputListener> Open Dialog command for " + cmdName );
              String deviceName = cmdName.split("_",2)[0];
              Device device = deviceManager.getDevice(deviceName);
              DataElement dataElement = device.getDataElement(cmdName.split("_",2)[1]);
              if ( dataElement != null ) {
                 ...
                 else if (origintag.contains("Sublwait") )
                    new DialogSetPoint(parent, origintag, device, dataElement, "Wait Time (x0.1) mn", true);
                 else if (origintag.contains("REmode") )
                    new DialogSetPoint(parent, origintag, device, dataElement, "Serial=0, Local=2", true);
                 ...
                 else if (origintag.contains("OPmode") &&  !origintag.contains("CTRL"))
                    new DialogSetPoint(parent, origintag, device, dataElement, "<html>Auto start/Recover auto=0,<br/>Auto start/Recover manual=1,<br/>Manual start/Recover auto=2,<br/>Manual start/Recover manual=3</html>", true);
