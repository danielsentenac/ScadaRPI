...
public class GlgGaugeGui extends GlgChildGui {
    ...
    protected void createAndShowGui () {
	  ...
	  content.add( glg_bean ); 
          ...
          glg_bean.SetSTag("Gename", origin, true);
    ...
    protected void createStatusMap(Hashtable < String, String > STATUS) {
       ...
       // Gauge Gui case
       STATUS.clear();
       STATUS.put("Gename", LINKSTATUS.get(origin));
       STATUS.put("Gestatus", LINKSTATUS.get(origin));
       STATUS.put("GePstatus", LINKSTATUS.get(origin + "Psub"));
       STATUS.put("GeVal", LINKSTATUS.get(origin + "Val"));
       STATUS.put("GeCol1", LINKSTATUS.get(origin + "Col"));
       STATUS.put("GePCol1", LINKSTATUS.get(origin + "PColsub"));
       STATUS.put("GeCol2", LINKSTATUS.get(origin + "Col"));
       STATUS.put("GeStr", LINKSTATUS.get(origin));
       STATUS.put("GePStr", LINKSTATUS.get(origin + "Psub"));
    }
	   ...
	   subaction = message_obj.GetSResource( "SubAction" );
           ...
           // Gauge command
              ...
              if ( device != null ) {
                 ...
                 // Update dataElement
                    logger.fine("GlgGaugeGui> set sensor ON for device = " + device.name + " gauge channel=" + cmdName.split("_",2)[1]);
