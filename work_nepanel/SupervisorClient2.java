    ...
    // SwingUtilities.invokeLater(new Runnable() {
     ...
     for (Map.Entry<String, String> e : PANEL2STATUS.entrySet()) {
         ...
         if ( !key.equals("")) {
            ...
            if (key.contains("PUSH") || key.contains("LN2VALVE") || key.contains("SIREN")) {
                 if ( lastIsVal == false ) 
                     addDataElement( new DataElement(name, value, DataType.READ_ONLY_STATUS,RegisterType.INT16,mbRegisterEnd+=1));
                 else
                     addDataElement( new DataElement(name, value, DataType.READ_ONLY_STATUS,RegisterType.INT16,mbRegisterEnd+=2));
     ...
     // GPS Time
     if ( lastIsVal == false ) 
         addDataElement( new DataElement(name, "GPS", DataType.READ_ONLY_VALUE,RegisterType.INT32,mbRegisterEnd+=1));
     else
         addDataElement( new DataElement(name, "GPS", DataType.READ_ONLY_VALUE,RegisterType.INT32,mbRegisterEnd+=2));
