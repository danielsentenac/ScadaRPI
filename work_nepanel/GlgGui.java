...
public class GlgGui extends JFrame implements ChannelList, Runnable  {
    ...
    private void initWebComponents() {
        ...
        panelweb.add(jfxPanel1, new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0
						     , GridBagConstraints.CENTER, GridBagConstraints.BOTH,
						     new Insets(1, 1, 1, 1), 0, 0));	
        panelweb.add(jfxPanel2, new GridBagConstraints(1, 0, 1, 1, 0.9, 1.0
						     , GridBagConstraints.CENTER, GridBagConstraints.BOTH,
						     new Insets(1, 1, 1, 1), 0, 0));	
    }
    ...
    private void createWebScene() {
        ...
        Platform.runLater(new Runnable() {
            @Override public void run() {
                ...
                // Safety NE + LEGEND scene (replaces former VIM webview)
                ViewData safetyFlagsNE = new ViewSafetyFlags("NESAFETYFLAGS", "SafetyFlagsNE");
                safetyFlagsNE.isSuspended = false;
                new Thread(safetyFlagsNE).start();

                StackPane safetyNEPane = createScaledNodePanel(safetyFlagsNE, "LASER BEAMS - NE", SAFETY_CONTENT_MARGIN);
                StackPane legendPane = createLegendPanel();

                HBox safetyRoot = new HBox(8, safetyNEPane, legendPane);
                safetyRoot.setStyle("-fx-background-color: black; -fx-padding: 8;");
                HBox.setHgrow(safetyNEPane, Priority.ALWAYS);
                HBox.setHgrow(legendPane, Priority.ALWAYS);
                safetyNEPane.prefWidthProperty().bind(safetyRoot.widthProperty().multiply(0.5));
    ...
    public void updateGui() {
       ...
       */
       ...
       if ( dev != null ) {
          ...
          if ( dcom.value == 1 ) {
              glg_bean2.SetGTag( "NEGeneralCol", new GlgPoint(0.5,0.5,0.5), true ); // General color Alarm
      	      glg_bean2.SetGTag( "NEZoneFrameCol", new GlgPoint(0.5,0.5,0.5), true ); // General Frame color Alarm
              glg_bean2.SetGTag( "NETunnelZoneCol", new GlgPoint(0.5,0.5,0.5), true ); // Wi color Alarm
              glg_bean2.SetGTag( "NETunnelZoneFrameCol", new GlgPoint(0.5,0.5,0.5), true ); // Wi Frame color Alarm
              glg_bean2.SetGTag( "NEBaseZoneCol", new GlgPoint(0.5,0.5,0.5), true ); // Inj color Alarm
              glg_bean2.SetGTag( "NEBaseZoneFrameCol", new GlgPoint(0.5,0.5,0.5), true ); // Inj Frame color Alarm
              glg_bean2.SetGTag( "NECryoZoneCol", new GlgPoint(0.5,0.5,0.5), true ); // Det color Alarm
              glg_bean2.SetGTag( "NECryoZoneFrameCol", new GlgPoint(0.5,0.5,0.5), true ); // Det Frame color Alarm
              glg_bean2.SetGTag( "NECleanZoneCol", new GlgPoint(0.5,0.5,0.5), true ); // Clean color Alarm
              glg_bean2.SetGTag( "NECleanZoneFrameCol", new GlgPoint(0.5,0.5,0.5), true ); // Clean Frame color Alarm
         ...
         if ( device != null ) {
		   ...
		   !glgName.contains("VALVE")) { // North End building        
	           ...
	           if ( status[2] == 1 || status[3] == 1 || status[4] == 1 || status[11] == 1 || status[15] == 1 ) { // Alarm (1,2,3 levels), Alarm to be resetted - Report
	               ...
	               if ( status[2] == 1 || status[3] == 1 || status[4] == 1) {
		           NEGeneralOKst = false;
			   NEGeneralStr = "DO NOT ENTER: CALL/CONTATTARE CONTROL ROOM / VAC";
		       }
    ...
    if (NEGeneralOKst == false) {
      ...
      glg_bean2.SetSTag( "NEGeneralStr", "NE BUILDING O2 SENSOR MONITORING : OK" , true ); // NE General OK
