...
public class DialogSetPoint extends JDialog {
    ...
    void jButtonOK_actionPerformed(ActionEvent e) {
        if (!jTextField.getText().equals("")) {
           logger.finer("Set new SetPoint for " + glgName + "=" + jTextField.getText());
	   // Update DataElement
           try {
             dataElement.setvalue = Double.parseDouble(jTextField.getText()); // Set setvalue to input
             device.commandSetQueue.add(dataElement); // queue new value
           }
           catch (NumberFormatException ex) {
               logger.log(Level.SEVERE,"DialogSetPoint::jButtonOK_actionPerformed> Not a double:" + jTextField.getText());
           }
	}
    }
