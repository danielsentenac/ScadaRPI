...
public class RgaHidenHal201RC extends Device {
   ...
   public RgaHidenHal201RC (String _name,
     ...
     addDataElement( new DataElement(name, "ELECTRONENERGY", DataType.READ_AND_WRITE_VALUE,RegisterType.FLOAT32,mbRegisterEnd+=1));
     addDataElement( new DataElement(name, "EMISSION", DataType.READ_AND_WRITE_VALUE,RegisterType.FLOAT32,mbRegisterEnd+=2));
     addDataElement( new DataElement(name, "MULTIPLIER", DataType.READ_AND_WRITE_VALUE,RegisterType.FLOAT32,mbRegisterEnd+=2));
     addDataElement( new DataElement(name, "FOCUS", DataType.READ_AND_WRITE_VALUE,RegisterType.FLOAT32,mbRegisterEnd+=2));
     addDataElement( new DataElement(name, "CAGE", DataType.READ_AND_WRITE_VALUE,RegisterType.FLOAT32,mbRegisterEnd+=2));
     ...
     logger.fine("RgaHidenHal201RC:RgaHidenHal201RC> " + name + " Modbus registers ends at offset " + mbRegisterEnd);
      ...
      logger.fine("RgaHidenHal201RC::RgaHidenHal201RC> Initialize device communication");
      try {
         initDevice();
      }
      catch (Exception ex) {
   ...
   public void updateDeviceData() {
     ...
     try {
        ...
        /****************************************************************************************/
        logger.finer("RgaHidenHal201RC:updateDeviceData> d1.value = " + (int) (d1.value));
        ...
        if ( d1.value == 0 ) { // RGA is not running
           ...
           if (serDataR == null ) {  // Com Status Error
              ...
              d2.value = Double.parseDouble(serDataR.split(" ")[0]);
           }
        }
        /****************************************************************************************/
                 ...
                 for (int i = 0; i < result.length; i++) {
                    if (result[i].contains(":")) {
                       ...
                       if (d != null) {
                          ...
                          d.value = Math.abs(Double.parseDouble(value));
                       }
                    }
                 }  
              }
      ...
      try {
         ...
         switch (choice) {
            case 1:
               ...
               serDataR = getAnswerFromDevice(serDataW);
               if ( serDataR != null )
                 d.value = Double.parseDouble(serDataR.split(" ")[0]);
               break;
            case 2:
   ...
   private String getAnswerFromDevice(String query) 
      ...
      logger.finer("RgaHidenHal201RC:getAnswerFromDevice> write to device:" + query + 
                     "(length=" + query.length() + ")");
      rs232.Write(query);
      Thread.sleep(500); // Essential to get good timing through communication channel
   ...
   private void resetDevice() {
        DataElement op = getDataElement("OPERATIONONOFF");
        op.value = 1;
        executeCommand (op);
        DataElement sc = getDataElement("RUNNINGONOFF");
        sc.value = 1;
        executeCommand (sc);
   }
   ...
   private void InitScanDevice() throws Exception {
      ...
      // Enable harware
       ...
       // Set HighDynamicRange option
       getAnswerFromDevice("sset options SaveScanDev:,BeamOnBefore:,BeamOffAfter:\r");
       // Set new scan row
       ...
       // Set stop mass
       getAnswerFromDevice("sset stop " + amuMax + "\r");
       // Set step mass
       getAnswerFromDevice("sset step 1.00\r");
       // Set low range limit
       getAnswerFromDevice("sset low -10\r");
       // Set high range limit
       getAnswerFromDevice("sset high -7\r");
       // Set current 
       getAnswerFromDevice("sset current -7\r");
       // Set dwell time
       getAnswerFromDevice("sset dwell 100%\r");
       // Set settle time
       ...
       // Set mass range
       getAnswerFromDevice("pset points " + amuMax + "\r");
   ...
   private void StopScanDevice() throws Exception {
      ...
      // Abort Scan
      getAnswerFromDevice("sset state Abort:\r");
      // RGA not RUNNING flag
      d.value = 0;
      // Stop recording data
