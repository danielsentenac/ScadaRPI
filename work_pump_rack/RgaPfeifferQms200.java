   ...
   public void updateDeviceData() {
     ...
     try {
        ...
        if ( d1.value == 1 ) { // RGA is started : collect data
           ...
           if (serDataR == null ) {  // Com Status Error
              setErrorComStatus();
              logger.log(Level.SEVERE, "2 - RGAPfeifferQms200:updateDeviceData> Communication with " + name + " interrupted");
              dcom.value = 1; // ERR COM
              // Try Re-Openning the device
              try {
                 rs232.Close();
        ...
        ex.printStackTrace();
        logger.log(Level.SEVERE, "3 - RGAPfeifferQms200>updateDeviceData:" + ex.getMessage());
        logger.log(Level.SEVERE, "3 - RGAPfeifferQms200:updateDeviceData> Communication with " + name + " interrupted");
        setErrorComStatus();
        dcom.value = 1; // ERR COM
        // Try Re-Openning the device
        try {
