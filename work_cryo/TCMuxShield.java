...
public class TCMuxShield extends Device {
   ...
   public void updateDeviceData() {
     ...
     logger.fine("TCMuxShield:updateDeviceData> GO!");
     try {
        byte ch = 0;
        for (int i = 0 ; i < temp.size(); i++) {
            logger.fine("TCMuxShield:updateDeviceData> ch = " + ch);
           ...
           logger.fine("TCMuxShield:updateDeviceData> Temp[" + ch + "]=" +  temp.elementAt(ch).value);
           if (++ch == 8) {
              temp.elementAt(ch).value = (float) rawIT / 256; // internal temperature reduced to quarter degree C
              logger.fine("TCMuxShield:updateDeviceData> INTERNAL Temp[" + ch + "]=" +  temp.elementAt(ch).value);
              ch = 0;
           }
        }
