...
public class GlgValveGui extends GlgChildGui {
    ...
    protected void createStatusMap(Hashtable < String, String > STATUS) {
       ...
       // Valve Gui case
       STATUS.clear();
       STATUS.put("Vname", LINKSTATUS.get(origin));
       STATUS.put("Vstatus", LINKSTATUS.get(origin));
       STATUS.put("VCol", LINKSTATUS.get(origin + "Col"));
       STATUS.put("VStr", LINKSTATUS.get(origin));
           ...
           // Valve command
              ...
              if (origintag.contains("Open")) {     
                 ...
                 if ( device != null ) {
                    ...
                    logger.finer("GlgValveGui:InputListener> Setting valve Open for device = " + device.name + " Valve name = " + dataElement.name);
