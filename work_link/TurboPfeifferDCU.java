   ...
   public void updateDeviceData() {
     ...
     try {
        for (int i = 0 ; i < 9 ; i++) {
           ...
           logger.finer("TurboPfeifferDCU:updateDeviceData> write to device name = " + name + ":" + i + " --> " + serDataW);
           ...
           logger.finer("TurboPfeifferDCU:updateDeviceData> reading from device name (" + i + ") = " + name + ":" + serDataR + "(length=" + serDataR.length() + ")");
           if (serDataR.equals("") || serDataR.length() != 20) {// Com Status Error
              com_error_count+=1;
              if ( com_error_count > MAX_COM_ERROR ) {
                 if ( hasWarned == false ) {
                    logger.log(Level.SEVERE, "TurboPfeifferDCU:updateDeviceData> Communication with " + name + " interrupted");
                 }
                 setErrorComStatus();
              }
              continue;
              ...
              if (serDataR.length() == 20) { // Data OK
                 ...
                 logger.finer("TurboPfeifferDCU:updateDeviceData> csum (" + i + ")" + " = " + csum%256);
                 int csumd = Integer.parseInt(serDataR.substring(serDataR.length() - 4, serDataR.length() - 1));
                 logger.finer("TurboPfeifferDCU:updateDeviceData> csumd (" + i + ")" + " = " + csumd);
                 if ( (csum%256) == csumd) { // Data OK
                    ...
                    if ( serDataR.substring(5, 8).equals(serDataW.substring(5, 8))) { // Channel OK
                       ...
                       if ( i < 4 ) { // short value
                          int result = 0;
                          logger.finer("TurboPfeifferDCU:updateDeviceData> name " + name + " ; result =" + serDataR.substring(10,16));
