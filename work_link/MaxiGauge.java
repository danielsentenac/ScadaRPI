...
public class MaxiGauge extends Device {
     ...
     try {
        for (int i = 0; i < 7; i++) {
           ...
           if (serDataR.length() <= 1 ) {// Com Status Error - reset all values -
              if ( hasWarned == false )
                 logger.log(Level.WARNING, "MaxiGauge:updateDexeviceData> Communication with " + name + " interrupted");
              setErrorComStatus();
              break;
           }
           else {
   ...
   public void executeCommand( DataElement e ) {
      ...
      logger.fine("MaxiGauge::executeCommand> Execute command/Write value "
                           + e.name + " value " + onOff);
      try {
         logger.fine("MaxiGauge::executeCommand> Send command/Write  " + serDataW);
         rs485.Write(serDataW);
         Thread.sleep(200); // Essential to get good timing through communication channel
         byte [] answer = null;
         while  (rs485.BytesAvailable() > 0 )
            answer = rs485.Read();
         logger.fine("MaxiGauge::executeCommand> Send command/Write  " + serDataACK);
         rs485.Write(serDataACK);
         Thread.sleep(200); // Essential to get good timing through communication channel
         while (rs485.BytesAvailable() > 0 )
             answer = rs485.Read();
