...
public class RgaPfeifferQms200 extends Device {
   ...
   public void updateDeviceData() {
     ...
     try {
        ...
        if ( d1.value == 255 ) { // RGA interrupted; Try to reconnect
           try {
              logger.fine("RGAPfeifferQms200::updateDeviceData> 1 - Re-initialize device...");
              rs232.Close();
              rs232.Open();
        ...
        if ( d1.value == 0 ) { // RGA is stopped
           ...
           if (serDataR == null ) {  // Com Status Error
              if (hasWarned == false) {
                 ...
                 logger.fine("RGAPfeifferQms200::updateDeviceData> Re-initialize device...");
              }
              setErrorComStatus();
              try {
                 rs232.Close();
        ...
        if ( d1.value == 1 ) { // RGA is started : collect data
           ...
           if (serDataR == null ) {  // Com Status Error
              if ( hasWarned == false ) {
                 ...
                 logger.fine("RGAPfeifferQms200::updateDeviceData> Re-initialize device and restart scan from beginning...");
              }
              setErrorComStatus();
              try {
                 rs232.Close();
              ...
              if ( hasWarned == true ) {
                 hasWarned = false;
                 logger.info("RGAPfeifferQms200:updateDeviceData> Communication with " + name + " back!");
              }
              DataElement d = null;
              if ( channelPos < 10 )
   ...
   public void executeCommand (DataElement e) {
      ...
      logger.fine("RGAPfeifferQms200:executeCommand> command = " + e.name);
