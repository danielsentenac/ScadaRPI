...
public class DialogSetPoint extends JDialog {
    ...
    private boolean okPressed = false; // NEW: did the user press OK?
    private double resultValue = 0.0;  // NEW: parsed value
    
    
   
    public DialogSetPoint(JFrame _parent, String _glgName, Device _device, DataElement _dataElement, String _title, boolean _modal) {
	try {
            parent = _parent;
            title = _title;
        ...
        // NEW: constructor for table input (no device, just numeric value)
        public DialogSetPoint(JFrame _parent,
                          String _title,
                          double initialValue,
                          boolean _modal) {
		try {
		    ...
		    title  = _title;
		    this.setModal(_modal);
		    glgName = _title;   // just for logging
		    device = null;
		    dataElement = null;

		    jButtonOK.setVisible(true);
		    jButtonOK.setEnabled(true);
		    Init();                    // builds UI and shows dialog (setVisible(true))

		    // pre-fill with current table value
		    jTextField.setText(Double.toString(initialValue));
		}
		catch (Exception ex) {
		    ex.printStackTrace();
		}
    ...
    void jButtonOK_actionPerformed(ActionEvent e) {
        ...
        logger.finer("Set new SetPoint for " + glgName + "=" + text);
        try {
            double val = Double.parseDouble(text.trim());

            // Always remember the value and that OK was pressed
            resultValue = val;
            okPressed = true;

            // Original behavior: update DataElement + queue command if we have a device
            if (device != null && dataElement != null) {
                dataElement.setvalue = val;          // Set setvalue to input
                device.commandSetQueue.add(dataElement); // queue new value
            }
        }
        catch (NumberFormatException ex) {
            logger.log(Level.SEVERE,
                       "DialogSetPoint::jButtonOK_actionPerformed> Not a double:" + text);
        }
    }
