...
public class RgaHidenHal201RC extends Device {
   ...
   public RgaHidenHal201RC (String _name, 
     ...
     logger.finer("RgaHidenHal201RC:RgaHidenHal201RC> " + name + " Modbus registers ends at offset " + mbRegisterEnd);

      // Instantiate communication channel and init device
      logger.finer("RgaHidenHal201RC::updateDeviceData> Initialize device communication");
      try {
         initDevice();
      }
      catch (Exception ex) {
        ...
        if ( d1.value == 0 ) { // RGA is not running
           ...
           if (serDataR == null ) {  // Com Status Error
              if (hasWarned == false) {
                 ...
                 logger.finer("RgaHidenHal201RC::updateDeviceData> Re-initialize device...");
              }
              setErrorComStatus();
              resetDevice();
           }
        ...
        if ( d1.value == 1 && d2.value == 1 ) { // RGA is running in operation : collect data
           ...
           if (serDataR == null ) {  // Com Status Error
              if ( hasWarned == false ) {
                 ...
                 logger.finer("RgaHidenHal201RC::updateDeviceData> Re-initialize device communication");
              }
              setErrorComStatus();
              resetDevice();
           }
   ...
   public void executeCommand (DataElement e) {
      ...
      logger.finer("RgaHidenHal201RC:executeCommand> command = " + e.name);
      int choice = 0;
