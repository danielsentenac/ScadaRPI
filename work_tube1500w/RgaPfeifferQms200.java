...
public class RgaPfeifferQms200 extends Device {
   ...
   public RgaPfeifferQms200 (String _name, 
     ...
     logger.finer("RGAPfeifferQms200:RGAPfeifferQms200> " + name + " Modbus registers ends at offset " + mbRegisterEnd);
     ...
     try {
       ...
       // Initialize RGA scan mode STAIR
       getAnswerFromDevice("MMO,2\r");
       // Initialize channel
       ...
       // Initialize Width AMU
       //getAnswerFromDevice("MWI,25\r");
       // Initialize RGA measuremnt speed 1 second
       //getAnswerFromDevice("MSD,10\r");
       // Initialize RGA first mass scan to 1
   ...
   public void updateDeviceData() {
     ...
     try {
        ...
        if ( d1.value == 255 ) { // RGA interrupted; Try to reconnect
           try {
              logger.finer("RGAPfeifferQms200::updateDeviceData> 1 - Re-initialize device...");
              rs232.Close();
              rs232.Open();
              String serDataR = null; 
              // Initialize communication in ASCII format
              serDataR = getAnswerFromDevice("CMO,1\r");
              // Initialize RGA with infinite Cycle
              serDataR = getAnswerFromDevice("CYS,0\r");
              // Initialize RGA scan mode STAIR
              serDataR = getAnswerFromDevice("MMO,2\r");
              // Initialize RGA in Faraday Mode
              serDataR = getAnswerFromDevice("SDT,0\r");
              // Initialize RGA SEM mode OFF
              serDataR = getAnswerFromDevice("SEM,0\r");
              // Initialize RGA first mass scan to 1
              serDataR = getAnswerFromDevice("MFM,1\r");
              // Initialize RGA Emission
              serDataR = getAnswerFromDevice("FIE,0\r");
              // Stop any running SCAN
              serDataR = getAnswerFromDevice("CRU,0\r");
              // Reinitialize values
              if (serDataR != null) 
        ...
        if ( d1.value == 0 ) { // RGA is stopped
           isStarted = false;
           // Test Communication
           String serDataW = "ERR\r"; // Get RGA error
           String serDataR = null; 
           ...
           if (serDataR == null ) {  // Com Status Error
              if (hasWarned == false) {
                 logger.log(Level.SEVERE, "RGAPfeifferQms200:updateDeviceData> Communication with " + name + " interrupted");
                 // Try Re-Openning the device
                 logger.finer("RGAPfeifferQms200::updateDeviceData> Re-initialize device...");
              }
              setErrorComStatus();
              try {
                 rs232.Close();
        ...
        if ( d1.value == 1 ) { // RGA is started : collect data
           if (isStarted == false) { 
              ...
              channelPos++;
              if (channelPos > 199) channelPos = 1;
           }
           ...
           if (serDataR == null ) {  // Com Status Error
              if ( hasWarned == false ) {
                 ...
                 logger.finer("RGAPfeifferQms200::updateDeviceData> Re-initialize device and restart scan from beginning...");
              }
              setErrorComStatus();
              try {
                 rs232.Close();
   ...
   public void executeCommand (DataElement e) {
      ...
      logger.finer("RGAPfeifferQms200:executeCommand> command = " + e.name);
   ...
   private String getAnswerFromDevice(String query) 
      ...
      byte [] answer = null;
      logger.finer("RGAPfeifferQms200:getAnswerFromDevice> write to device:" + query + 
                     "(length=" + query.length() + ")");
      rs232.Write(query);
      ...
      if ( answer != null ) {
         ...
         logger.finer("RGAPfeifferQms200:getAnswerFromDevice> answer from device:" + answerStr + 
