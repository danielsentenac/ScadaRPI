...
public class TitaneVarianTSP_32 extends Device implements DataTypes {
   ...
   public void updateDeviceData() {
     ...
     try {
        for (int i = 0 ; i < 16 ; i++) {
           ...
           if ( serDataR.length() < 2 || !xor_checksum(serDataR.substring(1)).equals(serDataR.charAt(serDataR.length() - 2)) ) {
              ...
              if ( com_error_count > MAX_COM_ERROR ) {
                 if ( hasWarned == false ) 
                    logger.log(Level.WARNING, "TitaneVarianTSP_32:updateDeviceData> Communication with " + name + " interrupted");
                 setErrorComStatus();
              }
              continue;
           }
              ...
              if ( hasWarned == true ) {
                 hasWarned = false;
                 logger.info("TitaneVarianTSP_32:updateDeviceData> Communication with " + name + " back!");
              }
              String resultStr = "";
              switch (dataType) {
                case 0: // Logic
        ...
        if ( hasWarned == false ) {
           logger.log(Level.WARNING, "TitaneVarianTSP_32:updateDeviceData> Communication with " + name + " interrupted");
           logger.log(Level.SEVERE, "TitaneVarianTSP_32:updateDeviceData>" + ex.getMessage());
        }
        setErrorComStatus();
     }
   }
