...
public class GlgValveGui extends GlgChildGui {
    ...
    protected void createStatusMap(Hashtable < String, String > STATUS) {
       ...
       // Valve Gui case
       STATUS.clear();
       STATUS.put("Vname", FLOWMETERSTATUS.get(origin));
       STATUS.put("Vstatus", FLOWMETERSTATUS.get(origin));
       STATUS.put("VCol", FLOWMETERSTATUS.get(origin + "Col"));
       STATUS.put("VStr", FLOWMETERSTATUS.get(origin));
	   ...
	   subaction = message_obj.GetSResource( "SubAction" );
           ...
           // Valve command
              ...
              if (origintag.contains("Open")) {  
                 logger.fine("GlgValveGui:InputListener> Setting valve Open");   
                 // Update dataElement
                 if ( device != null ) {
                    DataElement dataElement = device.getDataElement(cmdName.split("_",2)[1]);
                    logger.fine("GlgValveGui:InputListener> Setting valve Open for device = " + device.name + " Valve name = " + dataElement.name);
                 ...
                 // Update dataElement
                 if ( device != null ) {
                    logger.fine("GlgValveGui:InputListener> Setting valve Close");
                    DataElement dataElement = device.getDataElement(cmdName.split("_",2)[1]);
